...
import com.example.app.misc.objects.errorLog.ErrorLog
import com.example.app.ui.snackBar.MyEventType
import com.google.android.gms.common.api.CommonStatusCodes
import java.io.IOException

open class BtPrinter(private val activity: FragmentActivity, private val onEvent: (MyMsgEvent) -> Unit) :
    ...
    private fun initializePrinter() {
        ...
        if (mBluetoothAdapter == null) {
            sendEvent(context.getString(R.string.there_are_no_bluetooth_devices), MyEventType.ERROR)
        } else {
    ...
    override fun run() {
        try {
            if (ActivityCompat.checkSelfPermission(
                ...
                sendEvent(activity.getString(R.string.device_connected), MyEventType.INFO)
            } else {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    requestConnectPermission.launch(Manifest.permission.BLUETOOTH_CONNECT)
                }
            ...
            sendEvent(context.getString(R.string.error_connecting_device), MyEventType.ERROR)
            return
        }
    }
    ...
    private fun refreshBluetoothPrinter() {
        ...
        if (sv.useBtPrinter) {
            ...
            if (ActivityCompat.checkSelfPermission(
                ...
                onEvent(MyMsgEvent(context.getString(R.string.permission_denied), MyEventType.ERROR))
                return
            }
